...
public class LocalCookies {
	...
	@Contract(pure = true) public static ChromeBrowser home(final String userHome) {
		return home(new File(userHome));
	}

	@Contract(pure = true) public static ChromeBrowser home(final File userHome) {
		return config().chromeBrowser(userHome);
	}

	@Contract(pure = true) private static LocalCookies config() {
		return new LocalCookies();
	}

	@Contract(pure = true) private ChromeBrowser chromeBrowser() {
		return chromeBrowser(userHome);
	}

	@Contract(pure = true) private ChromeBrowser chromeBrowser(File userHome) {
		return new ChromeBrowser(userHome);
	}

	@Contract(pure = true) protected void userHome(final File userHome) {
