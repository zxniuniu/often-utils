...
public final class NetworkFileUtils {
    ...
    public enum Method {
        FILE("file"), // 配置文件下载
        FULL("full"),  //全量下载模式
        MULTITHREAD("multithread"),  // 多线程模式
        INTELLIGENT("intelligent"); // 智能多线程模式
     ...
     */
    @Contract(pure = true)
    public NetworkFileUtils method(final @NotNull NetworkFileUtils.Method method) {
        this.method = method;
        return this;
    }
    ...
    public int download(final @NotNull File folder) {
        ...
        switch (method) { // 配置信息
            ...
            case FULL, MULTITHREAD, INTELLIGENT -> {
                // 获取文件信息
        ...
        switch (method) {  // 开始下载
            ...
            case INTELLIGENT -> {
                executorService = Executors.newFixedThreadPool(MAX_THREADS); // 限制多线程;
                List<Integer> statusCodes = intelligent(0, fileSize / MAX_THREADS - 1);
                MultiThreadUtils.WaitForEnd(executorService); // 等待线程结束
                // 判断下载状态
                for (int statusCode : statusCodes) {
                    if (!URIUtils.statusIsOK(statusCode)) {
    ...
    private List<Integer> intelligent(int start, int end) {
        List<Integer> statusCodes = new CopyOnWriteArrayList<>();
        Response piece = JsoupUtils.connect(url).proxy(proxyHost, proxyPort).headers(headers).header("Range", "bytes=" + start + "-" + end).cookies(cookies).referrer(referrer).retry(retry, MILLISECONDS_SLEEP).retry(unlimitedRetry).GetResponse();
            ...
            for (int length; !Judge.isMinusOne(length = inputStream.read(buffer)); count += length) {
                ...
                if (count == 0 && end < fileSize - 1) {
                    executorService.execute(new Thread(() -> { // 执行多线程程
                        ...
                        statusCodes.addAll(intelligent(newStart, newEnd));
                    }));
                }
            }
