...
public class Yande_PopularDailyImages {
	...
	public static Map<String, String> GetHeatdayImagesInfo(String heatday_url) {
		...
		Document document = JsoupUtils.connect(heatday_url).timeout(10000).proxy(proxyHost, proxyPort).cookies(cookies).retry(MAX_RETRY, MILLISECONDS_SLEEP).errorExit(errorExit).GetDocument();
	...
	private static Map<String, String> GetParentImagesInfo(String parent_imageid, List<String> children_imageid_lists) {
		...
		Document document = JsoupUtils.connect(parentid_url).timeout(10000).proxy(proxyHost, proxyPort).cookies(cookies).retry(MAX_RETRY, MILLISECONDS_SLEEP).errorExit(errorExit).GetDocument();
		Elements posts = document.select("post");
