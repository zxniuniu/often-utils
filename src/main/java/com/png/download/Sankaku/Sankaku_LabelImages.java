...
public class Sankaku_LabelImages {
	...
	private static String GetImageUrl(String imageid) {
		Document labelurl_doc = JsoupUtils.connect(sankaku_url + "cn/post/show/" + imageid).timeout(12000).proxy(proxyHost, proxyPort).cookies(cookies).retry(MAX_RETRY, MILLISECONDS_SLEEP)
				.errorExit(errorExit).GetDocument();
	...
	private static Set<ThreeTuple<String, String, String>> GetLabelInfo(String whitelabel, String label_api_url) {
		...
		Document document = JsoupUtils.connect(label_api_url).timeout(12000).proxy(proxyHost, proxyPort).cookies(cookies).retry(MAX_RETRY, MILLISECONDS_SLEEP).errorExit(errorExit).GetDocument();
	...
	private static void download(String imageid, String imageUrl, String filename) {
		...
		NetworkFileUtils.connect(imageUrl).proxy(proxyHost, proxyPort).filename(filename).multithread(DOWN_THREADS).retry(MAX_RETRY, MILLISECONDS_SLEEP).errorExit(errorExit)
