...
public class Pixiv_LabelImages {
	...
	private static Set<ThreeTuple<String, List<String>, String>> get_label_infos(String whitelabel) {
		...
		Document document = JsoupUtils.connect(label_api_url).timeout(10000).proxy(proxyHost, proxyPort).cookies(cookies).retry(MAX_RETRY, MILLISECONDS_SLEEP).errorExit(errorExit).GetDocument();
	...
	private static Set<ThreeTuple<String, List<String>, String>> get_labelurl_infos(String label_api_url) {
		return get_doc_postinfos(JsoupUtils.connect(label_api_url).timeout(10000).proxy(proxyHost, proxyPort).cookies(cookies).retry(MAX_RETRY, MILLISECONDS_SLEEP).errorExit(errorExit).GetDocument());
	}
