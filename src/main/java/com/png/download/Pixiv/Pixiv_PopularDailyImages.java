...
public class Pixiv_PopularDailyImages {
	...
	private static Set<ThreeTuple<String, List<String>, String>> get_url_imagesInfo(String heatday_url) {
		return get_doc_postinfos(JsoupUtils.connect(heatday_url).timeout(10000).cookies(cookies).proxy(proxyHost, proxyPort).retry(MAX_RETRY, MILLISECONDS_SLEEP).errorExit(errorExit).GetDocument());
	}
