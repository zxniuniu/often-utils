...
public class Pixiv_Suggestion {
	...
	private static final boolean errorExit = App.errorExit; // 下载错误退出程序
	...
	public static void ImagesDownload() {
		...
		execute_program();
		System.out.println("下载 Pixiv 推荐图片 已完成 存储路径: " + image_folderPath);
	}

	private static void execute_program() {
		System.out.println("正在下载 Pixiv 推荐图片 存储路径: " + image_folderPath);
		String apiUrl = "https://www.pixiv.net/ajax/search/suggestion?mode=al";
		Document document = JsoupUtils.connect(apiUrl).timeout(10000).proxy(proxyHost, proxyPort).cookies(cookies).retry(MAX_RETRY, MILLISECONDS_SLEEP).errorExit(errorExit).GetDocument();
	...
	private static Set<ThreeTuple<String, List<String>, String>> get_doc_postinfos(Document document) {
		...
		JSONArray thumbnails_jsonArray = body_jsonObject.getJSONArray("thumbnails");// 获取数组
	...
	private static void download_imagefile(String imageid, List<String> fileurl_lists, String filename) {
		...
		for_fileurl_lists: for (String imagefile_url : fileurl_lists) {
			...
			for (int i = 0; i < MAX_RETRY; i++) {
				int statusCode = NetworkFileUtils.connect(new_imagefile_url).proxy(proxyHost, proxyPort).referrer(imageid_url).filename(new_filename).multithread(DOWN_THREADS)
						.download(image_folderPath);
				if (statusCode == HttpStatus.SC_NOT_FOUND) {
					if (suffix.equals(".jpg")) {
						suffix = ".png";
						new_filename = filenameBuilder + suffix;
